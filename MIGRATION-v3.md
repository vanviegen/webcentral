...
```ini
check_auth ${env:DASHBOARD_SECRET} { project_dashboard }   # DASHBOARD_SECRET=... in .env
respond 401
```
...
- **Move secrets into a `.env`.** `webcentral.conf` lives in the project directory and usually in
  git; a `.env` does not have to, and is read without a line asking for it (`env_file` reads any
  other). Its keys are `${env:KEY}`, and a bare `KEY` in an `env`
